...
impl Display for VoiceUdpError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match &self.kind {
            VoiceUdpErrorType::Bind => f.write_str("could not bind local UDP socket"),
            VoiceUdpErrorType::Connect => f.write_str("could not connect to the voice UDP socket"),
...
pub enum VoiceUdpErrorType {
    ...
    /// Could not bind to the UDP socket.
    Bind,

    /// Could not connect to the voice UDP socket.
    Connect,
